...
/*   Updated: 2022/12/02 13:44:51 by htahvana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @return t_bool 
 */
static t_bool	apply_occlusion(t_rayhit *hit, int x, int occlusion, t_limit *y)
{
...
void	draw_wall(t_app *app, int x, t_rayhit *hit, int occlusion_type)
{
	...
	while (y.start < y.end)
	{
		...
		if (y.start % 2 == 0)
			app->depthmap[y.start / 2][x] = depth;
		y.start++;
	}
}
